-- 🧱 BrickBox 최종 완전한 더미 데이터 정리 스크립트
...
WHERE model_name LIKE 'brickbox_model_%' 
   ...
   OR created_by = 'mock_user'
   OR created_by = 'fake_user'
...
WHERE metrics::text LIKE '%"mAP50": 0.95%'
   ...
   OR metrics::text LIKE '%"accuracy": 0.85%'
   OR metrics::text LIKE '%"accuracy": 0.78%'
   OR metrics::text LIKE '%"accuracy": 0.88%'
   OR metrics::text LIKE '%"accuracy": 0.93%';
...
WHERE performance_metrics::text LIKE '%"inference_time": 0.045%'
   ...
   OR performance_metrics::text LIKE '%"fps": 25%'
   OR performance_metrics::text LIKE '%"fps": 15%'
   OR performance_metrics::text LIKE '%"fps": 18%'
   OR performance_metrics::text LIKE '%"fps": 35%';
...
DELETE FROM recognition_results WHERE confidence_score = 0.75;
DELETE FROM recognition_results WHERE confidence_score = 0.88;
DELETE FROM recognition_results WHERE confidence_score = 0.93;
DELETE FROM recognition_results WHERE confidence_score = 0.82;
DELETE FROM recognition_results WHERE confidence_score = 0.86;
DELETE FROM recognition_results WHERE confidence_score = 0.91;
...
DELETE FROM training_jobs WHERE job_name LIKE 'mock_training_%';
DELETE FROM training_jobs WHERE job_name LIKE 'fake_training_%';
DELETE FROM training_jobs WHERE job_name LIKE 'example_training_%';
DELETE FROM training_jobs WHERE job_name LIKE 'simulation_training_%';
DELETE FROM training_metrics WHERE training_job_id IN (
  SELECT id FROM training_jobs WHERE job_name LIKE 'test_%' 
    ...
    OR job_name LIKE 'mock_%'
    OR job_name LIKE 'fake_%'
    OR job_name LIKE 'example_%'
    OR job_name LIKE 'simulation_%'
);
...
DELETE FROM automation_config WHERE config_key LIKE '%test%' 
  ...
  OR config_key LIKE '%mock%'
  OR config_key LIKE '%fake%'
  OR config_key LIKE '%example%'
  OR config_key LIKE '%simulation%';
...
BEGIN
    ...
    RAISE NOTICE '🧹 최종 완전한 더미 데이터 정리 완료!';
    RAISE NOTICE '📊 남은 데이터:';
    ...
    RAISE NOTICE '💎 더 이상 더미 데이터는 존재하지 않습니다!';
    RAISE NOTICE '🌟 완전히 깨끗한 시스템이 준비되었습니다!';
END $$;
