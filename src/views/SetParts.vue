...
export default {
  ...
  setup() {
    ...
    const paginatedStoreSetsData = ref([]) // 현재 페이지에 표시할 레고 세트
    ...
    const inventorySetNumbers = computed(() => {
      try {
        ...
        if (!isPleyonUser.value && userLegoSets.value && userLegoSets.value.length > 0) {
          ...
          console.log('[SetParts] 일반회원 레고 세트 번호 생성 완료:', setNumbers.size, '개')
          return setNumbers
        }
        
        // 플레이온 동기화 계정인 경우: 플레이온 인벤토리 사용
        console.log('[SetParts] inventorySetNumbers computed 실행:', { 
        ...
        if (storeInventory.value && Array.isArray(storeInventory.value) && storeInventory.value.length > 0) {
          ...
          console.log('[SetParts] inventorySetNumbers 생성 완료:', setNumbers.size, '개')
        } else {
          console.log('[SetParts] inventorySetNumbers: storeInventory가 비어있음')
        }
        return setNumbers
      } catch (error) {
        console.error('[SetParts] inventorySetNumbers computed error:', error)
        return new Set()
      }
    })
    ...
    const GENERAL_LIST_CACHE_KEY = 'brickbox:set-parts:general-list-cache:v1' // 🔧 수정됨
    ...
    const loadFirstPageStoreSets = async (start, end) => {
      ...
      // 테마 정보는 필요한 만큼만 조회 (세트 데이터와 병렬 처리)
      ...
      // 테마 정보 조회를 병렬로 처리 (이미 데이터가 있으므로 병렬 가능)
      const themePromise = missingThemeIds.length > 0
        ? supabase
            .from('lego_themes')
      ...
      // 테마 정보 로드 완료 대기
      const { data: themesData } = await themePromise
      if (themesData) {
    ...
    const searchSets = async () => {
      ...
      try {
        ...
        if (searchInStoreOnly.value && user.value && inventorySetNumbers.value && inventorySetNumbers.value.size > 0) {
          ...
          console.log(`[SetParts] 매장 보유 세트 필터링: ${results.length}개`)
        }
    ...
    const loadAllStoreSetsParts = async () => {
      const targetSets = await fetchAllSetsForCurrentFilter()
      ...
      loadingStoreSetsProgress.value = { current: 0, total: targetSets.length }
      ...
      try {
        ...
        for (let i = 0; i < targetSets.length; i += CONCURRENT_LIMIT) {
          const batch = targetSets.slice(i, i + CONCURRENT_LIMIT)
          ...
          loadingStoreSetsProgress.value.current = Math.min(i + CONCURRENT_LIMIT, targetSets.length)
        }
    ...
    const loadUserLegoSets = async () => {
      ...
      try {
        ...
        if (fetchError) {
          console.error('[SetParts] 일반회원 레고 세트 로드 실패:', fetchError)
          userLegoSets.value = []
          return
        }

        userLegoSets.value = data || []
        console.log('[SetParts] 일반회원 레고 세트 로드 완료:', userLegoSets.value.length, '개')
      } catch (err) {
        console.error('[SetParts] 일반회원 레고 세트 로드 오류:', err)
        userLegoSets.value = []
      }
    }
    ...
    const fetchAllSetsForCurrentFilter = async () => {
      if (!searchInStoreOnly.value) {
        ...
        return enriched.map(mapSetRecord).filter(Boolean)
      }
      ...
      if (!isPleyonUser.value) {
        return (userLegoSets.value || []).map(mapSetRecord).filter(Boolean)
      }
    ...
    let activeStoreLoadPromise = null

    const loadPaginatedStoreSets = async () => {
      if (activeStoreLoadPromise) {
        await activeStoreLoadPromise
      }
      ...
      const loadTask = (async () => {
        ...
        try {
          if (searchInStoreOnly.value) {
            ...
            if (!storeInventoryCachePromise) {
              rebuildStoreInventoryCache().catch(err => {
                console.error('[SetParts] 백그라운드 캐시 빌드 실패:', err)
              })
            }
            return
          }
          ...
          console.error('[SetParts] 레고 세트 로드 실패:', err)
          paginatedStoreSetsData.value = []
          if (!searchInStoreOnly.value) {
            storeSetsCountValue.value = 0
          }
      ...
      activeStoreLoadPromise = loadTask
      try {
        await loadTask
      } finally {
        activeStoreLoadPromise = null
      }
    }

    // 페이지네이션
    const totalPages = computed(() => {
      return Math.max(1, Math.ceil(storeSetsCount.value / itemsPerPage))
    })
    ...
    const openSetPartsModal = async (set) => {
      ...
      try {
        ...
        // 부품 정보와 색상, 이미지 데이터를 병렬로 조회 // 🔧 수정됨
        const partIds = [...new Set(nonSpareParts.map(p => p.part_id).filter(Boolean))]
        const colorIds = [...new Set(nonSpareParts.map(p => p.color_id).filter(id => id !== null && id !== undefined))]
        const elementIds = [...new Set(nonSpareParts.map(p => p.element_id).filter(Boolean))].map(id => String(id))
        const partIdsForImages = [...new Set(nonSpareParts.map(p => p.part_id).filter(Boolean))]
        const colorIdsForImages = [...new Set(nonSpareParts.map(p => p.color_id).filter(id => id !== null && id !== undefined))]
        const hasElementIds = elementIds.length > 0
        const hasPartColorImages = partIdsForImages.length > 0 && colorIdsForImages.length > 0
        ...
        const [
          ...
          partIds.length > 0 ? supabase
            .from('lego_parts')
            .select('part_num, name, part_img_url')
            .in('part_num', partIds) : Promise.resolve({ data: [], error: null }), // 🔧 수정됨
          colorIds.length > 0 ? supabase
            .from('lego_colors')
            .select('color_id, name, rgb')
            .in('color_id', colorIds) : Promise.resolve({ data: [], error: null }), // 🔧 수정됨
          hasElementIds ? supabase
            .from('part_images')
            .select('element_id, uploaded_url')
            .in('element_id', elementIds)
            .not('uploaded_url', 'is', null) : Promise.resolve({ data: [], error: null }), // 🔧 수정됨
          hasElementIds ? supabase
            .from('image_metadata')
            .select('element_id, supabase_url')
            .in('element_id', elementIds)
            .not('supabase_url', 'is', null) : Promise.resolve({ data: [], error: null }), // 🔧 수정됨
          hasPartColorImages ? supabase
            .from('part_images')
            .select('part_id, color_id, uploaded_url')
            .in('part_id', partIdsForImages)
            .in('color_id', colorIdsForImages)
        ...
        if (partsInfoResult.error) throw partsInfoResult.error
        if (colorsInfoResult.error) throw colorsInfoResult.error
        if (elementImagesResult.error) throw elementImagesResult.error // 🔧 수정됨
        if (elementMetadataResult.error) throw elementMetadataResult.error // 🔧 수정됨
        if (partImagesResult.error) throw partImagesResult.error // 🔧 수정됨
        if (metadataImagesResult.error) throw metadataImagesResult.error // 🔧 수정됨

        const partsInfo = partsInfoResult.data || []
        const colorsInfo = colorsInfoResult.data || []
        const elementImages = elementImagesResult.data || [] // 🔧 수정됨
        const elementMetadata = elementMetadataResult.data || [] // 🔧 수정됨
        const partImages = partImagesResult.data || [] // 🔧 수정됨
        const metadataImages = metadataImagesResult.data || [] // 🔧 수정됨
        ...
        if (elementMetadata.length > 0) {
          for (const img of elementMetadata) {
    ...
    watch(user, async (newUser, oldUser) => {
      ...
      // 동일한 사용자면 스킵
      if (newUser?.email === oldUser?.email && !newUser === !oldUser) {
        return
      }
...
@media (max-width: 1024px) {
  ...
  .modal-parts-grid .part-card .part-image {
    padding: 0.25rem !important;
  }
}
