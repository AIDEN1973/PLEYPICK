<template>
  <div class="quality-healing-dashboard">
    ...
    <div class="dashboard-header">
      <h1>📊 품질 회복 자동률 대시보드</h1>
      <p class="subtitle">QA FAIL → 재렌더링 → PASS 자동 회복률 분석</p>
      <div class="dashboard-info">
        <span class="info-badge">실시간 모니터링</span>
        <span class="info-badge">자동 회복률 추적</span>
        <span class="info-badge">품질 개선 분석</span>
      </div>
    </div>
...
const loadOverallStats = async () => {
  try {
    ...
    const successfulChecks = data.filter(log => 
      log.qa_status === 'pass' || log.qa_status === 'PASS'
    ).length
    
    overallStats.value = {
      total_qa_checks: totalChecks,
      successful_checks: successfulChecks,
      overall_success_rate: totalChecks > 0 ? Math.round((successfulChecks / totalChecks) * 100) : 0,
      avg_ssim: data.length > 0 ? data.reduce((sum, log) => sum + (log.ssim || 0), 0) / data.length : 0
    }
  } catch (error) {
...
const loadRetryStats = async () => {
  try {
    ...
    const { data, error } = await supabase
      .from('qa_logs')
      .select('part_id, qa_status, ssim, snr, sharpness, reprojection_error')
      .gte('timestamp', new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString())
    
    if (error) throw error
    
    // 부품별로 그룹화하여 재시도가 있었던 부품들 찾기
    const partGroups = {}
    data.forEach(log => {
      if (!partGroups[log.part_id]) {
        partGroups[log.part_id] = []
      }
      partGroups[log.part_id].push(log)
    })
    
    const retryParts = Object.values(partGroups).filter(logs => logs.length > 1)
    const totalRetries = retryParts.reduce((sum, logs) => sum + logs.length, 0)
    const successfulRetries = retryParts.reduce((sum, logs) => 
      sum + logs.filter(log => log.qa_status === 'pass' || log.qa_status === 'PASS').length, 0
    )
...
const loadFailureAnalysis = async () => {
  try {
    ...
    const { data, error } = await supabase
      .from('qa_logs')
      .select('ssim, snr, sharpness, reprojection_error, qa_status')
      .gte('timestamp', new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString())
    ...
    // 실패 원인별 분석
    const failureTypes = {
      'fail_ssim': { total: 0, recovered: 0, avg_ssim: 0, avg_snr: 0 },
      'fail_sharpness': { total: 0, recovered: 0, avg_ssim: 0, avg_snr: 0 },
      'fail_pnp': { total: 0, recovered: 0, avg_ssim: 0, avg_snr: 0 },
      'fail_snr': { total: 0, recovered: 0, avg_ssim: 0, avg_snr: 0 }
    }
    
    data.forEach(log => {
      let failureType = null
      if (log.ssim < 0.96) failureType = 'fail_ssim'
      else if (log.sharpness < 0.7) failureType = 'fail_sharpness'
      else if (log.reprojection_error > 1.5) failureType = 'fail_pnp'
      else if (log.snr < 30) failureType = 'fail_snr'
      
      if (failureType) {
        failureTypes[failureType].total++
        if (log.qa_status === 'pass' || log.qa_status === 'PASS') {
          failureTypes[failureType].recovered++
        }
        failureTypes[failureType].avg_ssim += log.ssim || 0
        failureTypes[failureType].avg_snr += log.snr || 0
      }
    })
    
    // 결과 변환
    failureAnalysis.value = Object.entries(failureTypes).map(([type, stats]) => ({
      failure_type: type,
      total_failures: stats.total,
      recovered_failures: stats.recovered,
      recovery_rate_percent: stats.total > 0 ? Math.round((stats.recovered / stats.total) * 100) : 0,
      avg_ssim: stats.total > 0 ? stats.avg_ssim / stats.total : 0,
      avg_snr: stats.total > 0 ? stats.avg_snr / stats.total : 0
    })).filter(item => item.total_failures > 0)
    
  } catch (error) {
    console.error('실패 분석 로드 실패:', error)
    failureAnalysis.value = []
  }
}

const loadPartHistory = async () => {
  try {
    // qa_logs에서 부품별 히스토리 계산
    const { data, error } = await supabase
      .from('qa_logs')
      .select('part_id, element_id, qa_status, ssim, snr, sharpness, reprojection_error, timestamp')
      .gte('timestamp', new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString())
      .order('part_id')
    ...
    // 재시도가 있었던 부품들만 필터링
    const retryParts = Object.entries(partGroups)
      .filter(([partId, logs]) => logs.length > 1)
      .map(([partId, logs]) => {
        const successfulAttempts = logs.filter(log => 
          log.qa_status === 'pass' || log.qa_status === 'PASS'
        ).length
        const avgSsim = logs.reduce((sum, log) => sum + (log.ssim || 0), 0) / logs.length
        const lastAttempt = logs[logs.length - 1].timestamp
        
        return {
          part_id: partId,
          element_id: logs[0].element_id,
          total_attempts: logs.length,
          successful_attempts: successfulAttempts,
          healing_rate_percent: Math.round((successfulAttempts / logs.length) * 100),
          avg_ssim: avgSsim,
          last_attempt: lastAttempt
        }
      })
      .sort((a, b) => b.healing_rate_percent - a.healing_rate_percent)
      .slice(0, 20)
    
    partHistory.value = retryParts
...
const loadTrendData = async () => {
  try {
    ...
    const { data, error } = await supabase
      .from('qa_logs')
      .select('timestamp, qa_status, ssim, snr, sharpness, reprojection_error')
    ...
    // 일별 통계 계산
    trendData.value = Object.entries(dailyGroups).map(([date, logs]) => {
      ...
      return {
        ...
        successful_attempts: successfulAttempts,
        daily_healing_rate: totalAttempts > 0 ? Math.round((successfulAttempts / totalAttempts) * 100) : 0,
        avg_ssim: avgSsim
      }
