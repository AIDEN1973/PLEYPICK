export default async function handler(req, res) {
  ...
  try {
    // URL에서 경로 추출 (예: /api/lego-instructions/ko-kr/service/buildinginstructions/31159)
    const fullPath = req.url || ''
    const pathMatch = fullPath.match(/\/api\/lego-instructions\/(.+)/)
    const pathString = pathMatch ? pathMatch[1].split('?')[0] : ''
    
    console.log(`[LEGO Instructions API] 경로 파싱:`, { fullPath, pathString })
