...
def create_test_webp():
    ...
    for i in range(0, 1024, 64):
        for j in range(0, 1024, 64):
            if (i // 64 + j // 64) % 2 == 0:
                # 사각형 그리기
                for x in range(i, min(i + 32, 1024)):
                    for y in range(j, min(j + 32, 1024)):
                        test_image.putpixel((x, y), (255, 0, 0))
    
    # 테스트 디렉토리 생성
    test_dir = "test_webp_metadata"
    os.makedirs(test_dir, exist_ok=True)
    
    # 원본 WebP 파일 생성 (메타데이터 없음)
    original_webp = os.path.join(test_dir, "test_original.webp")
    test_image.save(original_webp, "WEBP", quality=90, method=6)
    print(f"원본 WebP 파일 생성: {original_webp}")
...
def test_webp_metadata_injection():
    ...
    try:
        # 1. cwebp 방식 테스트
        print("\n1. cwebp 방식 테스트")
        success1 = test_cwebp_method(test_file)
        
        # 2. PIL 방식 테스트
        print("\n2. PIL 방식 테스트")
        shutil.copy2(backup_file, test_file)  # 원본 복원
        success2 = test_pil_method(test_file)
        
        # 결과 요약
        print("\n" + "=" * 80)
        print("테스트 결과 요약")
        print("=" * 80)
        print(f"cwebp 방식: {'성공' if success1 else '실패'}")
        print(f"PIL 방식: {'성공' if success2 else '실패'}")
