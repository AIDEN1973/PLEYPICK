#!/usr/bin/env python3
"""
Supabase Storage의 models 버킷에 있는 모델 파일들을 model_registry에 등록
"""
...
def list_storage_models(supabase: Client):
    """Storage의 models 버킷에서 모델 파일 목록 조회"""
    try:
        # models 버킷의 모든 파일 조회
        ...
        print(f"📁 Storage에서 발견된 모델 파일: {len(model_files)}개")
        ...
        print(f"❌ Storage 파일 목록 조회 실패: {e}")
        return []

def register_model_to_database(supabase: Client, model_info):
    """모델을 model_registry에 등록"""
    try:
        # 파일명에서 모델 정보 추출
        filename = model_info['name']
        
        # 모델명과 버전 추출 (예: set_76917-1_best.pt -> set_76917-1_best, v1.0.0)
        ...
        # 공개 URL 생성
        public_url = supabase.storage.from_('models').get_public_url(model_info['path'])
        ...
        if result.data:
            print(f"✅ 모델 등록 완료: {model_name} ({model_version})")
            return True
        else:
...
def main():
    """메인 함수"""
    print("🔍 Supabase Storage 모델 파일 등록 시작")
    print("=" * 50)
    ...
    model_files = list_storage_models(supabase)
    if not model_files:
        print("📭 Storage에 모델 파일이 없습니다.")
        return
    
    # 각 모델을 데이터베이스에 등록
    registered_count = 0
    for model_info in model_files:
        if register_model_to_database(supabase, model_info):
            registered_count += 1
