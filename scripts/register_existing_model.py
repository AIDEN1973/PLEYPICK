#!/usr/bin/env python3
"""
기존 Storage 모델을 model_registry에 등록
"""
...
def register_existing_model(supabase: Client):
    """기존 모델을 model_registry에 등록"""
    try:
        # Storage에서 파일 정보 확인
        ...
        for file_info in files:
            ...
            if filename and filename != 'brickbox_s_seg_20251010_034224':
                continue
                
            print(f"📁 발견된 모델 파일: {filename}")
            ...
            if filename.endswith('.onnx'):
                ...
                model_type = 'yolo_segmentation'  # 기본값
            ...
            model_data = {
                ...
                'version': model_version,  # version 컬럼도 추가
                'model_type': model_type,
                'model_path': public_url,
                'model_url': public_url,  # model_url 컬럼도 추가
                'pt_model_path': public_url,
                'is_active': True,  # 첫 번째 모델이므로 활성화
                'performance_metrics': {
                    'mAP50': 0.0,
                    'mAP50_95': 0.0,
                    'precision': 0.0,
            ...
            if result.data:
                ...
                print(f"   - 활성화: {'예' if model_data['is_active'] else '아니오'}")
                return True
            else:
                print(f"❌ 모델 등록 실패")
                return False
...
def main():
    ...
    # 기존 모델 등록
    success = register_existing_model(supabase)
