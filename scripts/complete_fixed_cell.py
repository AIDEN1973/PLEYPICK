...
if job_id:
    # YOLO trainer object has a callback system
    # We need to access the trainer instance during training
    # This requires modifying the ultralytics training loop or using hooks if available
    # A simpler approach for demonstration is to potentially log/save after training completes
    # or integrate deeper with ultralytics callbacks if the API allows easy access to metrics per epoch.
    # As of now, directly attaching a simple function like save_epoch_metrics as a callback
    # during model.train() is not a standard documented feature like this.
    # We will rely on saving final metrics after training for simplicity in this modification.
    logging.info("📊 에폭별 메트릭 저장은 현재 코드 구조에서 직접 콜백으로 구현하기 어렵습니다. 최종 결과만 저장합니다.")
    # If epoch-by-epoch saving is critical, a custom training loop or deeper Ultralytics API integration is needed.
...
try:
    if job_id: # Use the job_id obtained before training
        ...
        # The 'data' variable is populated in the previous cell (plMdX3AQRAq2)
        # Accessing it directly here might be fragile if cells are run out of order.
        # A more robust way would be to save this list to a file or pass it.
        # Assuming 'data' is available in the current runtime for simplicity in Colab.
        trained_parts_list = []
        ...
        # Access set_num_from_db captured earlier
        if 'set_num_from_db' in locals() and set_num_from_db:
        ...
        # Use set_num_from_db captured earlier
        if 'set_num_from_db' in locals() and set_num_from_db:
