#!/usr/bin/env python3
"""
🧱 BrickBox Blender 테스트 렌더링 스크립트

Blender 내부에서 실행되는 간단한 테스트 스크립트
- LDraw 파일 로드 테스트
- 기본 렌더링 테스트
- 로컬 파일 저장
"""
...
def load_ldraw_part(part_path):
    """LDraw 부품 로드"""
    try:
        # LDraw Importer Add-on 자동 활성화 (Blender 4.5.3 LTS 최적화)
        import addon_utils
        
        # Add-on 활성화 (default_set=True로 설정 저장)
        try:
            addon_utils.enable("ImportLDraw-master", default_set=True)
            print("✅ LDraw Importer Add-on 활성화 완료 (설정 저장됨)")
        except Exception as e:
            print(f"⚠️ Add-on 활성화 실패: {e}")
            return None
        
        # LDraw 환경 변수 설정 (경로 문제 해결)
        import os
        os.environ['LDRAWDIR'] = 'C:/LDraw'
        print("✅ LDraw 환경 변수 설정: C:/LDraw")
        
        # LDraw 파일 임포트 (올바른 operator 사용)
        ...
        if not imported_objects:
            print("❌ LDraw 파일에서 메시를 찾을 수 없습니다")
            return None
        ...
        print(f"✅ LDraw 부품 로드 성공: {part_object.name}")
        return part_object
...
def apply_random_material(part_object):
    """랜덤 재질 적용"""
...
def main():
    """메인 실행 함수"""
    print("🧱 BrickBox Blender 테스트 렌더링 시작")
    ...
    # LDraw 부품 로드
    part_path = "C:/LDraw/parts/3001.dat"
    part_object = load_ldraw_part(part_path)
    ...
    # 랜덤 재질 적용
    color = apply_random_material(part_object)
    ...
    # 렌더링
    output_path = os.path.join(output_dir, "test_3001.png")
