...
def deploy_model_to_supabase():
    ...
    try:
        ...
        print("⚠️ python-dotenv가 설치되지 않았습니다. 수동으로 환경 변수를 로드합니다.")
        # .env 파일 직접 읽기
        env_vars = {}
        try:
    ...
    SUPABASE_KEY = os.getenv('VITE_SUPABASE_ANON_KEY') or env_vars.get('VITE_SUPABASE_ANON_KEY')
    ...
    for model_file in model_files:
        ...
        try:
            ...
            if result:
                ...
                print(f"📁 파일 경로: yolo/{model_file}")
                print(f"📊 파일 크기: {len(file_data) / 1024 / 1024:.2f} MB")
                
                # 공개 URL 생성
                public_url = f"{SUPABASE_URL}/storage/v1/object/public/models/yolo/{model_file}"
